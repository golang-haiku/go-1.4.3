// mksyscall_solaris.pl syscall_haiku.go syscall_haiku_amd64.go
...
var (
	...
	procFdopendir    = modlibc.NewProc("fdopendir")
	procReaddir_r    = modlibc.NewProc("readdir_r")
	procClosedir     = modlibc.NewProc("closedir")
)
...
func Wait4(pid int, wstatus *WaitStatus, options uintptr, rusage *Rusage) (wpid int, err error) {
	r0, _, e1 := sysvicall6(procwaitpid.Addr(), 4, uintptr(pid), uintptr(unsafe.Pointer(wstatus)), uintptr(options), uintptr(unsafe.Pointer(rusage)), 0, 0)
	wpid = int(r0)
	if e1 != 0 {
		err = e1
	}
